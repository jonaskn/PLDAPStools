function result = saccadeDetector(time,trace)
...
potential_saccades([0 diff(potential_saccades)]<min_isi)=[];
...
%     endindex(rem)=[];
%     startindex(rem)=[];
    if endindex(iSaccade)+blink_isi > size(speed,2) || isnan(sum(speed(endindex(iSaccade)+(1:blink_isi)))) || isnan(sum(speed(startindex(iSaccade)-(1:blink_isi))))
        startindex(iSaccade) = potential_saccades(iSaccade);
        endindex(iSaccade) = potential_saccades(iSaccade);
        continue;
    end
